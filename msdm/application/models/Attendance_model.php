...
class Attendance_model extends CI_Model
{
    ...
    public function em_attendance()
    {
      ...
      FROM `attendance`
      LEFT JOIN `employee` ON `attendance`.`emp_id`=`employee`.`em_id` WHERE `attendance`.`status` = 'A' ORDER BY `attendance`.`id` DESC";
    ...
    public function getPINFromID($employee_ID) {
      $sql = "SELECT `em_id` FROM `employee`
    ...
    {
      $sql    = "SELECT `attendance`.*,
      `employee`.`em_id`, CONCAT(`first_name`, ' ', `last_name`) AS name,`em_id`, TRUNCATE((ABS(( TIME_TO_SEC( TIMEDIFF( `signin_time`, `signout_time` ) ) )))/3600, 1) AS Hours
      FROM `attendance`
      LEFT JOIN `employee` ON `attendance`.`emp_id` = `employee`.`em_id` 
